...
def write_to_markdown(projects):
    ...
    for p in projects:
        ...
        _matrix.append([
            ...
            p.last_utime.replace(microsecond=0).isoformat(),
            p.last_dtime.replace(microsecond=0).isoformat() if p.last_dtime is not None else 'None',
            f'{calculate_uptime(p):.3f} (since {p.ctime})'
        ])
